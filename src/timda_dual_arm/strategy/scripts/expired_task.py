...
class ExpiredTask:
    ...
    def get_obj_inf(self, side):
        ...
        for _id, mat, name, exp, side_id in zip(ids, mats, names, exps, side_ids):
            ...
            obj['side_id'] = side_id
            obj['pos'] = mat[0:3, 3]
            obj['vector'] = mat[0:3, 2] #rotation (aruco z 軸)
            obj['sucang'], roll = self.dual_arm.suc2vector(mat[0:3, 2], [0, 1.57, 0]) #suck ang(0-90), roll (7軸)
            obj['euler']   = [roll, 90, 0] 
            if obj['vector'][2] > -0.2: #(aruco z 軸) goood
                self.object_queue.put(obj)
                print('fuck+++++============--------------', obj['pos'])
            else:#bad vector !!from down to too
    ...
    def strategy(self, state, side):
        ...
        if state == State.init:
            ...
            cmd['pos'], cmd['euler'], cmd['phi'] = c_pose[side][c_pose[side+'_indx']][0], c_pose[side][c_pose[side+'_indx']][1], 0  #phi 7-axis
            cmd_queue.put(copy.deepcopy(cmd))
            cmd['suc_cmd'] = 0
            cmd['cmd'] = 'occupied' #ask yu
            ...
            if self.retry_obj_queue[side].empty() and self.target_obj_queue[side].empty():
                ...
                for _ in range(self.object_queue.qsize()):
                    ...
                    if self.obj_done[obj['id']] == False:
                        if side == 'left' and obj['pos'][1] < -0.02: #y<-0.02 (2cm) too
                            self.object_queue.put(obj)
                            continue
